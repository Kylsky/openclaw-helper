...
function humanizeErrorMessage(message) {
  ...
  if (raw === "更新失败：检测到 Windows 文件占用（EBUSY）。请先关闭正在运行的 OpenClaw/网关，再重试更新。") {
    return "更新失败：有文件正在被占用。请先关闭正在运行的 OpenClaw 或网关，再重试更新。";
  }
...
function humanizeLogLine(line) {
  ...
  if (raw === "[warn] 检测到当前 OpenClaw 不支持 --non-interactive，正在自动使用兼容参数重试更新…") {
    return "检测到当前 OpenClaw 版本较旧，正在自动改用兼容参数重新执行更新…";
  }
  if (raw === "[update] 检测到更新前网关正在运行；为避免 Windows 文件占用，先临时停止网关，更新完成后请手动启动或重启。") {
    return "检测到网关正在运行；为避免文件占用，正在先停止网关，更新完成后请手动启动或重启。";
  }
  if (raw === "[update] 已停止正在运行的网关，继续执行更新。") {
    return "已停止正在运行的网关，正在继续执行更新。";
  }
  if (raw === "[warn] 未确认到可停止的网关进程；若稍后仍报文件占用，请先手动停止网关后重试。") {
    return "暂时没能确认到可停止的网关进程；如果后面仍提示文件占用，请先手动停止网关再重试。";
  }
  if (raw === "[update] 已使用兼容参数完成更新命令，继续检查是否需要额外升级步骤…") {
    return "更新命令已用兼容方式重试成功，正在继续检查是否还需要额外升级步骤…";
  }
  if (raw === "[update] 已完成兼容修复，继续执行剩余收尾步骤…") {
    return "兼容修复已完成，正在继续执行剩余收尾步骤…";
  }
  if (raw === "[update] 已完成兼容修复，跳过重复健康检查。") {
    return "兼容修复已经完成，不再重复执行健康检查。";
  }
  if (raw === "[update] 更新前网关未运行，跳过自动重启。") {
    return "更新前网关没有在运行，已跳过自动重启。";
  }
  if (raw === "[update] 更新前网关未运行。为避免弹出终端，助手不会自动启动网关；如需使用新版本，请稍后手动启动或重启网关。") {
    return "为避免弹出终端，助手不会自动启动网关；如需使用新版本，请稍后手动启动或重启。";
  }
  if (raw === "[update] 更新前检测到网关正在运行，尝试自动重启…") {
    return "更新前网关正在运行，正在尝试自动重启…";
  }
  if (raw === "[update] 更新前检测到网关正在运行。为避免弹出终端，已跳过自动重启；请稍后手动重启网关以应用新版本。") {
    return "为避免弹出终端，已跳过自动重启网关；如需应用新版本，请稍后手动重启。";
  }
  if (raw === "[update] 网关已自动重启。") {
    return "网关已自动重启。";
  }
  if (raw === "[update] 自动更新流程完成。") {
    return "更新的收尾步骤已完成。";
  }
  if (raw === "[weixin] 检测到本地已存在插件目录，跳过远程安装并直接修复：C:\\Users\\75205\\.OpenClaw\\extensions\\openclaw-weixin") {
    return "检测到本地已经有微信插件，正在跳过远程安装并直接修复。";
  }
  match = raw.match(/^\[weixin\] 检测到本地已存在插件目录，跳过远程安装并直接修复：(.+)$/);
  if (match) return "检测到本地已经有微信插件，正在跳过远程安装并直接修复。";
  if (raw === "[weixin] 远程安装未完成，但检测到本地插件目录仍可修复，继续使用本地插件...") {
    return "远程安装没有完成，但本地插件还在，正在继续按本地修复处理。";
  }
